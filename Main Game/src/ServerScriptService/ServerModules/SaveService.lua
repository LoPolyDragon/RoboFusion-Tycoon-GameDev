...
function M:Load(plr, defaultData)
	...
	if not ok or not d then d = table.clone(defaultData) end
...
function M:Save(uid, dataTbl)
	local s = sessions[uid]; if not s then return end
	s.data = dataTbl or s.data
	if os.clock() - s.last < 6 then return end          -- 防洪
...
task.spawn(function()
	while true do
		for uid in pairs(sessions) do M:Save(uid) end
		task.wait(AUTO_INT)
	end
end)
