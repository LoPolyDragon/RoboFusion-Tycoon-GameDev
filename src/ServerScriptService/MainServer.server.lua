--========================================================
-- MainServer.lua   （ServerScriptService 直挂 Script）
-- 负责：RemoteEvent / RemoteFunction 转发、
--      冷却、防刷、背包推送、自动收集
--========================================================
...
local MineService = require(ServerModules.MineService)
...
Players.PlayerAdded:Connect(function(plr)
	...
	if not data then
		plr:Kick("Data load failed, please re‑join.");  return
	end
	...
	local function push(id, qty)
		if qty and qty > 0 then table.insert(inv, { itemId = id, quantity = qty }) end
	end

	if type(data.BotInventory) == "table" then
		for id, qty in pairs(data.BotInventory) do  push(id, qty) end
		data.BotInventory = nil                     -- 清掉旧字段
	end
	if type(data.ShellInventory) == "table" then
		for id, qty in pairs(data.ShellInventory) do push(id, qty) end
	...
	ReplicatedStorage.RemoteEvents.UpdateInventoryEvent
		:FireClient(plr, GameLogic.GetInventoryDict(plr))
end)
...
local function onCD(uid, key, gap)
	local now = os.clock()
	local t   = cooldowns[uid] or {}; cooldowns[uid] = t
	if (now - (t[key] or 0)) < gap then return false end
	t[key] = now;  return true
end
...
CollectScrapEvt.OnServerEvent:Connect(function(p, amt)
	if typeof(amt) ~= "number" or not onCD(p.UserId, "collect", 1) then return end
	GameLogic.AddScrap(p, math.clamp(amt, 1, 5))
end)

GenerateBotEvt.OnServerEvent:Connect(function(p, cost)
	if typeof(cost) ~= "number" or not onCD(p.UserId, "genShell", 1.5) then return end
	GameLogic.GenerateBotShell(p, math.clamp(cost, 1, 50))
end)

ShipBotEvt.OnServerEvent:Connect(function(p, botId, qty)
	if typeof(botId) ~= "string" or typeof(qty) ~= "number" then return end
	qty = math.floor(qty)
	if qty <= 0 or qty > 1000 or not onCD(p.UserId, "ship", 2) then return end

	local ok, msg = GameLogic.ShipBots(p, botId, qty)
	ShipBotEvt:FireClient(p, ok, msg)
	if ok then UpdateInvEvt:FireClient(p, GameLogic.GetInventoryDict(p)) end
end)

UpgradeMachineEvt.OnServerEvent:Connect(GameLogic.UpgradeMachine)
...
local function getSp(t, l) return t[l] or t[#t] end

GetUpgradeInfoF.OnServerInvoke = function(p, machine)
	local d = GameLogic.GetPlayerData(p); if not d then return end
	local lv = d.Upgrades[machine .. "Level"] or 1
	local t  = speedTbl[machine]; if not t then return end
	return {
		level     = lv,
		speed     = getSp(t, lv),
...
local function ownsPass(plr)
	local ok, res = pcall(MarketplaceService.UserOwnsGamePassAsync,
		MarketplaceService, plr.UserId, AUTO_PASS_ID)
	return ok and res
end
